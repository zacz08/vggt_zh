...
def predictions_to_glb(
    ...
    if camera_matrices.ndim == 4 and camera_matrices.shape[0] == 1:
        camera_matrices = camera_matrices[0]
...
def segment_sky(image_or_path, onnx_session, mask_filename=None):
    """
    Segments sky from an image using an ONNX model.
    Thanks for the great model provided by https://github.com/xiongzhu666/Sky-Segmentation-and-Post-processing

    Args:
        image_or_path: Path to input image (str) or in-memory RGB image (np.ndarray)
    ...
    if isinstance(image_or_path, str):
        assert os.path.isfile(image_or_path), f"File not found: {image_or_path}"
        image = cv2.imread(image_or_path)
        if image is None:
            raise ValueError(f"Failed to read image from {image_or_path}")
    elif isinstance(image_or_path, np.ndarray):
        image = image_or_path.copy()
    else:
        raise ValueError("image_or_path must be a file path or numpy array")
