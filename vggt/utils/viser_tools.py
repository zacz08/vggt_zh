...
from utils.bev_handler import bev_gaussian_splat
...
_STATE = {
    ...
    "bev_cloud": None,          # handle for BEV point cloud layer
}
...
def _extract_scene_data(
    ...
    Args:
        ...
        with_bev (bool): Whether to include BEV features in the visualization.
    """
    ...
    if with_bev:
        ...
        init_threshold_val = torch.quantile(conf_flat, init_conf_threshold / 100.0)
        init_conf_mask = (conf_flat >= init_threshold_val) & (conf_flat > 0.1)
        points = points_centered[init_conf_mask]
        colors = colors_flat[init_conf_mask]
        conf = conf_flat[init_conf_mask]
        bev_feature = bev_gaussian_splat(points, colors, conf, cfg).squeeze(0)  # (C, H, W)
    ...
    return {
        ...
        "bev": bev_feature if with_bev else None  # (B, C, H, W) or None
    }
...
def _bev_to_pointcloud(bev, cfg, *,
                       mode: str = "rgb",
                       z_plane: float = 0.0,
                       lift_by_z: bool = False,
                       z_lift_scale: float = 1.0,
                       stride: int = 1,
                       min_weight: float = 0.0) -> tuple[np.ndarray, np.ndarray]:
    """
    Convert BEV feature (C,H,W) to a point cloud (N,3) with per-point colors (N,3).
    - mode: "rgb" | "z_mean" | "weight" | "rgb*weight"
    - z_plane: base z (meters) for plotting the BEV layer
    - lift_by_z: if True, add (z_mean*scale) to z_plane to create gentle 3D relief
    - stride: subsampling stride to speed up rendering
    - min_weight: drop pixels whose weight_sum < threshold (in original BEV scale)
    """
    if isinstance(bev, torch.Tensor):
        bev = bev.detach().cpu().numpy()
    # --- accept (C,H,W) or legacy (1,C,H,W) ---
    if bev.ndim == 4 and bev.shape[0] == 1:
        bev = bev[0]  # -> (C,H,W)

    C, H, W = bev.shape
    # channels: [r_mean, g_mean, b_mean, z_mean, weight_sum, (optional others...)]
    r = bev[0]; g = bev[1]; b = bev[2]
    z_mean = bev[3]
    weight = bev[4]

    # Subsample mask & weight threshold
    grid_y = np.arange(0, H, stride)
    grid_x = np.arange(0, W, stride)
    yy, xx = np.meshgrid(grid_y, grid_x, indexing="ij")  # (H',W')
    r_ = r[yy, xx]; g_ = g[yy, xx]; b_ = b[yy, xx]
    z_ = z_mean[yy, xx]; w_ = weight[yy, xx]

    valid = (w_ >= min_weight)
    if valid.sum() == 0:
        return np.zeros((0,3), dtype=np.float32), np.zeros((0,3), dtype=np.uint8)

    r_, g_, b_, z_, w_, yy, xx = r_[valid], g_[valid], b_[valid], z_[valid], w_[valid], yy[valid], xx[valid]

    # Compute world coordinates for BEV pixel centers
    x_min, x_max, dx = cfg.lift.x_bound
    y_min, y_max, dy = cfg.lift.y_bound
    
    # u = (y_max - y)/dy  →  y = y_max - (u + 0.5)*dy
    ys = y_max - (xx + 0.5) * dy

    # v = (H - 1) - (x - x_min)/dx  →  x = x_min + (H - 1 - (v + 0.5)) * dx
    xs = x_min + ((H - 1) - (yy + 0.5)) * dx

    if lift_by_z:
        # z_mean is in [0,1] normalized by (z_min,z_max). Lift gently in meters.
        points_z = z_plane + (z_.astype(np.float32) * z_lift_scale)
    else:
        points_z = np.full_like(xs, fill_value=z_plane, dtype=np.float32)

    points = np.stack([xs.astype(np.float32),
                       ys.astype(np.float32),
                       points_z], axis=-1)  # (N,3)

    # Colors
    if mode == "rgb":
        cols = np.stack([r_, g_, b_], axis=-1)
        cols = np.clip(cols, 0.0, 1.0)
        colors = (cols * 255).astype(np.uint8)
    elif mode == "rgb*weight":
        w01 = w_ / (w_.max() + 1e-6)
        cols = np.stack([r_*w01, g_*w01, b_*w01], axis=-1)
        cols = np.clip(cols, 0.0, 1.0)
        colors = (cols * 255).astype(np.uint8)
    elif mode == "z_mean":
        colors = _apply_colormap_jet(z_)
    elif mode == "weight":
        w01 = w_ / (w_.max() + 1e-6)
        colors = _apply_colormap_jet(w01)
    else:
...
def viser_init(pred_dict: dict,
    ...
    # --- Add BEV layer as a point cloud if present ---
    bev = data.get("bev", None)
    if bev is not None:
        bev_points, bev_colors = _bev_to_pointcloud(
            bev, cfg,
            mode="rgb*weight",
            z_plane=0.0,
            lift_by_z=False,
        ...
        bev_pc = server.scene.add_point_cloud(
            name="bev_layer",
            points=bev_points,
            colors=bev_colors,
            point_size=0.5,
        ...
        _STATE["bev_cloud"] = bev_pc

        # GUI controls for BEV
        gui_show_bev   = server.gui.add_checkbox("Show BEV", initial_value=True)
        gui_bev_mode   = server.gui.add_dropdown("BEV Mode",
                               options=["rgb", "rgb*weight", "z_mean", "weight"],
                               initial_value="rgb*weight")
        gui_bev_stride = server.gui.add_slider("BEV Stride", min=1, max=8, step=1, initial_value=2)
        gui_bev_wthr   = server.gui.add_slider("BEV Min Weight", min=0.0, max=float(bev[0,4].max()+1e-6),
                                               step=float(max((bev[0,4].max()/200),1e-6)), initial_value=0.0)
        gui_bev_lift   = server.gui.add_checkbox("Lift by z_mean", initial_value=False)
        gui_bev_lift_s = server.gui.add_slider("z Lift Scale (m)", min=0.0, max=3.0, step=0.1, initial_value=1.0)

        # store GUI handles
        _STATE["gui"].update({
            "show_bev": gui_show_bev,
            "bev_mode": gui_bev_mode,
            "bev_stride": gui_bev_stride,
            "bev_weight_thr": gui_bev_wthr,
            "bev_lift": gui_bev_lift,
            "bev_lift_scale": gui_bev_lift_s,
        })

        @gui_show_bev.on_update
        def _(_evt) -> None:
            if _STATE["bev_cloud"] is not None:
                _STATE["bev_cloud"].visible = gui_show_bev.value

        def _refresh_bev(_evt=None):
            if _STATE["bev_cloud"] is None:
                return
            bev_points, bev_colors = _bev_to_pointcloud(
                _STATE["data"]["bev"], cfg,
                mode=_STATE["gui"]["bev_mode"].value,
                z_plane=0.0,
                lift_by_z=_STATE["gui"]["bev_lift"].value,
                z_lift_scale=_STATE["gui"]["bev_lift_scale"].value,
                stride=int(_STATE["gui"]["bev_stride"].value),
                min_weight=float(_STATE["gui"]["bev_weight_thr"].value),
            )
            _STATE["bev_cloud"].points = bev_points
            _STATE["bev_cloud"].colors = bev_colors

        @gui_bev_mode.on_update
        def _evt_mode(evt): _refresh_bev(evt)
        @gui_bev_stride.on_update
        def _evt_stride(evt): _refresh_bev(evt)
        @gui_bev_wthr.on_update
        def _evt_wthr(evt): _refresh_bev(evt)
        @gui_bev_lift.on_update
        def _evt_lift(evt): _refresh_bev(evt)
        @gui_bev_lift_s.on_update
        def _evt_lifts(evt): _refresh_bev(evt)
...
def viser_update(
    ...
    *,
    ...
    # If BEV layer already exists, always compute BEV this frame ---
    compute_bev = _STATE.get("bev_cloud") is not None
    ...
    # Update BEV cloud if exists
    if _STATE["bev_cloud"] is not None and _STATE["data"].get("bev", None) is not None:
        gui = _STATE["gui"]
        bev_points, bev_colors = _bev_to_pointcloud(
            _STATE["data"]["bev"], cfg,
            mode=gui["bev_mode"].value,
            z_plane=0.0,
            lift_by_z=gui["bev_lift"].value,
            z_lift_scale=gui["bev_lift_scale"].value,
            stride=int(gui["bev_stride"].value),
            min_weight=float(gui["bev_weight_thr"].value),
        )
        _STATE["bev_cloud"].points = bev_points
        _STATE["bev_cloud"].colors = bev_colors
    return server
